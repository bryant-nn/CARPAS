...
def predict_num_aspects(article: str, model: str = "gpt-4o", temperature: float = 0.7):
    prompt = f"""
You are given a epidemic report article. Estimate how many distinct, non-overlapping aspects or key themes are discussed in the article. Only return a single integer representing the number of aspects.
...
"""
        ...
        prompt = f"""
You are given a epidemic article and an initial list of section titles (called "aspects"). Your task is to revise this list to better reflect the key themes and content of the article.
...
def process_files(folder_path: List[str], all_aspects: List[str], output_path: str, use_memory: bool, y: int, negative: int, 
    ...
    for file_path in tqdm(folder_path):
        ...
        if provide_aspect_num:
            key_file_path = f"../Synthetic_data/{file_path}"
