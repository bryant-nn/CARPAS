...
def check_coverage_fn(state: SummaryState):
    ...
    prompt = f"""
You are given an article and a list of aspect titles. Your task is to decide whether these aspects better reflect the key themes or main topics.
...
def process_files(folder_path: List[str], all_aspects: List[str], output_path: str, use_memory: bool, y: int, negative: int, 
    ...
    for file_path in tqdm(folder_path):
        ...
        if provide_aspect_num:
            if file_path in aspect_predictions_map:
                predicted_aspect_num = aspect_predictions_map[file_path]
...
if __name__ == "__main__":
    ...
    process_files(train_files, all_aspects, output_folder, use_memory=args.memory, y=args.y, negative=args.n, provide_aspect_num=args.provide_aspect_num, aspect_predictions_map=aspect_predictions_map)
