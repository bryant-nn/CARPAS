...
def calculate_bertscore(reference_summaries, generated_summaries, cuda=0):
    scorer = BERTScorer(model_type="microsoft/deberta-v3-large", lang="en", device=f'cuda:{cuda}')
...
def evaluate_summary(ground_truth, predicted, cuda=0) -> dict:
    ...
    for i, p in enumerate(predicted):
        for j, g in enumerate(ground_truth):
            if p == "" or g == "":
                ...
                # Calculate BERTScore
                bert_score_df = calculate_bertscore([g], [p], cuda)  # Wrap in list to handle one-to-one pair
    ...
    for i, j in zip(row_ind, col_ind):
        ...
        if p != "" and g != "":
            # BERTScore
            bert_score_df = calculate_bertscore([g], [p], cuda)
