...
class Config:
    ...
    data_base_dir = "../Synthetic_data/synthetic_final_dataset"
...
def load_data_from_pointer_file(pointer_file_path: str, base_dir: str) -> list[dict]:
    ...
    with open(f"{pointer_file_path}", 'r', encoding='utf-8') as f:
...
def main_worker(rank, world_size, config):
    ...
    train_samples = load_data_from_pointer_file(config.train_pointer_file, f"{config.data_base_dir}/train")
    valid_samples = load_data_from_pointer_file(config.test_pointer_file, f"{config.data_base_dir}/test")
    ...
    # Note: When using DistributedSampler, do NOT set shuffle=True in DataLoader
    ...
    # 2. Initialize model and wrap with DDP
    ...
    for epoch in range(config.num_epochs):
        ...
        # ADDED: Log epoch-level training loss to TensorBoard
        if rank == 0:
...
def run_testing(config):
    ...
    test_samples = load_data_from_pointer_file(config.test_pointer_file, f"{config.data_base_dir}/test")
